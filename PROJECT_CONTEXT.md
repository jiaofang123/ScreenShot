...
- 2026-06-23 修复记录：
  ...
  - 2026-06-27 修复记录：
    - macOS 启动后会主动请求屏幕录制权限；截图开始前只做兜底检查，若仍未授权则停在主窗口提示，不再创建冻结覆盖层，避免权限弹窗被覆盖。
    - 针对 macOS 安装包（Release/DMG）补充覆盖层兜底显示逻辑：由 Rust 端主动显示并聚焦覆盖层窗口，不再完全依赖隐藏窗口中的前端自我唤起。
    - 覆盖层在快照尚未注入前显示轻量 `正在准备截图…`，避免打包版主窗口隐藏后直接落回桌面、看起来像“只切到桌面没有进入截图”。
    - macOS 打包启用 ad-hoc app bundle 签名（`signingIdentity: "-"`），让安装包内的 `ScreenShot.app` 使用 `com.amamda.screenshot` 作为签名标识并封存资源，减少 TCC 权限与实际运行身份错位。
- 构建验证：
  - `npm run build` 通过。
